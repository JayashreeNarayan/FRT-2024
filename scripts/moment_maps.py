...
if __name__ == "__main__":
    ...
    LOS_labels = [r"$\left(\begin{array}{c} 1 \\ 0 \\ 0 \end{array}\right) $", r"$\left(\begin{array}{c} 0 \\ 1 \\ 0 \end{array}\right) $", r"$\left(\begin{array}{c} 0 \\ 0 \\ 1 \end{array}\right) $"]
        ...
        if action == choices[0]:
            ...
            for file in files:
                ...
                if file == "SMM_0.0_0.0.npy": # Since the 2nd moment map needs different plot variables
                    ...
                    cfp.plot(x=0.05, y=0.75, text=r"1st-moment"+"\n"+img_names[1]+"\n"+r"$\sigma$ = "+str(sigma)+r"$~\mathrm{km\,s^{-1}}$", axes_format=["",None], fontsize='small', backgroundcolor="white", transform=plt.gca().transAxes)
                ...
                if file == "FMM_0.0_0.0.npy": # 2nd and 0th moment map is not to be smoothed or gaussian corrected 
                    ...
                    cfp.plot(x=0.05, y=0.75, text=r"Low-pass-filtered 1st-moment"+"\n"+img_names[1]+"\n"+r"$\sigma$ = "+str(sigma)+r"$~\mathrm{km\,s^{-1}}$", backgroundcolor="white", fontsize='small', transform=plt.gca().transAxes)
                    t = plt.text(0.85, 0.8, LOS_labels[2] , transform=plt.gca().transAxes) # for LOS
                    t.set_bbox(dict(facecolor='white', alpha=0.3, linewidth=0))
            ...
            for file in files:
                ...
                for imom, moment_map in enumerate(moment_maps):
                    ...
                    if file == "PPV_0_0.npy": # To plot the right labels
                        t = plt.text(0.03, 0.9, img_names[0] , transform=plt.gca().transAxes)
                        t.set_bbox(dict(facecolor='white', alpha=0.3, linewidth=0))
                        t = plt.text(0.8, 0.85, LOS_labels[2] , transform=plt.gca().transAxes) # for LOS
                        t.set_bbox(dict(facecolor='white', alpha=0.3, linewidth=0))                        
                ...
                cfp.plot(x=0.05, y=0.75, text=r"1st-moment"+"\n"+img_names[0]+"\n"+r"$\sigma$ = "+str(sigma)+r"$~\mathrm{km\,s^{-1}}$",  fontsize='small', backgroundcolor="white", transform=plt.gca().transAxes)
                t = plt.text(0.85, 0.8, LOS_labels[2] , transform=plt.gca().transAxes) # for LOS
                t.set_bbox(dict(facecolor='white', alpha=0.3, linewidth=0))
                ...
                cfp.plot(x=0.05, y=0.75, text=r"Low-pass-filtered 1st-moment"+"\n"+img_names[0]+"\n"+r"$\sigma$ = "+str(sigma)+r"$~\mathrm{km\,s^{-1}}$", axes_format=[None,""], backgroundcolor="white", fontsize='small', transform=plt.gca().transAxes)
                t = plt.text(0.85, 0.8, LOS_labels[2] , transform=plt.gca().transAxes) # for LOS
                t.set_bbox(dict(facecolor='white', alpha=0.3, linewidth=0))
                cfp.plot(save=outpath+file[:-4]+"_"+moment_maps[1]+"_corrected_PDF.pdf", xlabel=cmap_labels[1], ylabel="PDF", fontsize='small', ylog=True, xlim=[xmin, xmax], ylim=[1.e-2,5.e1])
