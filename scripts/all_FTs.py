#!/usr/bin/env python
# This file plots the power spectra of the moment maps at 1tff (Fig. 4) and at SE (Fig. 7) -- TOTAL 3 IMAGES
...
def FT_1tff_before(): # plots Fig. 4 top panel
...
def FT_1tff_after(): # plots Fig. 4 bottom panel
    path='all_FTs/' # save these figures to a different subfolder
    if not os.path.isdir(outpath+path):
        cfp.run_shell_command('mkdir '+outpath+path)
    FT_obj = [] 
